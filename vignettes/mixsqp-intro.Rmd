...
Optionally, load the `Rmosek` package. (The Rmosek package is only
used to check the version of MOSEK.)

```{r load-rmosek, message=FALSE}
library(Rmosek)
```
...
If you used `REBayes` and `Rmosek` above, it is also helpful to record
the version of MOSEK that was used.
