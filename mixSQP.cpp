...
List mixSQP       (const arma::mat& L, const arma::vec& x0,
  ...
  arma::vec  r(k);   // Vector of temporary variable mimicking b
  ...
  for (j = 0; j < maxqpiter; j++) {
    ...
    if (arma::norm(p,2) < convtol) {
      ...
      // Find an index with smallest multiplier in the active set, Add this to the inactive set
      r.elem(ind).fill(1);
      newind     = r.index_min();
      t[newind]  = 1;
      ind        = find(t);
