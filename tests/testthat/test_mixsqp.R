...
# In this example, the number of EM iterations is increased to 20.
# Ideally, mix-SQP would converge with a smaller number of EM
# iterations. Therefore, this is a "difficult" example for mix-SQP.
test_that("mix-SQP works for difficult ashr example with binomial likelihood",{

  # This example was generated by running this code:
  #
  # library(ashr)
  #   set.seed(1)
  ...
  skip_if_mixkwdual_doesnt_work()
  capture.output(out1 <- mixsqp(L,control = list(numiter.em = 20)))
  out2 <- mixkwdual(L)
  expect_equal(out1$value,out2$value,tolerance = 1e-8,scale = 1)
  expect_equal(out1$x,out2$x,tolerance = 1e-4,scale = 1)
})

# In this example, the "eps" control parameter is increased to 1e-6.
# Ideally, mix-SQP would also converge with a smaller value of "eps".
# Therefore, this is a "difficult" example for mix-SQP.
#
# This test comes from stephens999/ashr Issue #76.
test_that("mix-SQP works for a difficult mashr example",{
  load("mashr.nicolegay.example.RData")
  skip_if_mixkwdual_doesnt_work()
  capture.output(out1 <- mixsqp(L,w,control = list(eps = 1e-6)))
  out2 <- mixkwdual(L,w)
  expect_equal(out1$value,out2$value,tolerance = 1e-8,scale = 1)
  expect_equal(out1$x,out2$x,tolerance = 1e-4,scale = 1)
})
